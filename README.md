...
- `docs/` – product specs (master: `docs/Phase1_MVP-Spec_v0.4.md`).
...
- Test locally: `curl -s http://localhost:3000/api/health | jq`. Expect `status: "ok"` with a count once DB is reachable; otherwise `status: "degraded"`.
